...
get_device_path(io_object_t& serial_port) {
  ...
  if (callout_path) {
    if (CFGetTypeID(callout_path) == CFStringGetTypeID()) device_path = cfstring_to_string(static_cast<CFStringRef>(callout_path));
...
get_string_property(io_object_t& device, const char* property) {
  ...
  if (device) {
    ...
    if (name_as_cfstring) {
      if (CFGetTypeID(name_as_cfstring) == CFStringGetTypeID()) property_name = cfstring_to_string(static_cast<CFStringRef>(name_as_cfstring));
